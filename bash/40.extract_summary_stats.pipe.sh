...
GNM=$(ls ${genome}|rev|cut -d"/" -f1|rev)
echo -e "Reference_genome\t"${GNM} >> ${out_file}
...
# calculate SPLITE-LOOP count
if [ $SPLT = "Y" ]
then
echo -e "Host-Host_Loops\t"$(cat BASIC.DIR/${LIB}.HG38B.HH.cluster|wc -l| xargs printf "%'.f") >> ${out_file}
echo -e "EBV-EBV_Loops\t"$(cat BASIC.DIR/${LIB}.HG38B.EE.cluster|wc -l| xargs printf "%'.f") >> ${out_file}
echo -e "Host-EBV_Loops\t"$(cat BASIC.DIR/${LIB}.HG38B.HE-E.anchor|wc -l| xargs printf "%'.f") >> ${out_file}
fi
